...
import { Bell, CalendarClock, Check, ChevronDown, CircleHelp, Clock3, FileCheck2, Info, ListChecks, Quote, RotateCcw, ShieldCheck, TriangleAlert } from "lucide-react";
import type { Assessment } from "@/lib/types";
import { translate, translateCompanyEvidence, type Locale } from "@/lib/i18n";
import { Badge, StatusBadge, formatDate } from "./ui";

type Props = { assessment: Assessment; mode: "demo" | "live"; analyzedAt: string; stale: boolean; tab: string; setTab: (tab: string) => void; now: string; onRerun: () => void; isLoading: boolean; locale: Locale };

export function AssessmentResults({ assessment, mode, analyzedAt, stale, tab, setTab, now, onRerun, isLoading, locale }: Props) {
  ...
  return <>
    <div className="results-title">
      ...
      <span className="analyzed-at"><Check size={13} /> {mode === "demo" ? tx("Demo analysis", "Демоанализ") : tx("AI analysis", "ИИ-анализ")} · {formatDate(analyzedAt, true, locale)}</span>
    </div>
    ...
    <div className="panel result-panel">
      ...
      <div id={`panel-${tab}`} role="tabpanel" aria-labelledby={`tab-${tab}`} className="tab-content" tabIndex={0}>
        {tab === "requirements" && <>
          ...
          {assessment.requirements.length ? assessment.requirements.map((r, index) => <details className="requirement-row" key={index}>
